## read given path (reference) and recorded path (real) from scaled data
## plot in one figure
#
# Z. Zhang
# 2023/1
...
for path_id in range(num_path):
    ...
    ax.plot3D(ref_x_global,ref_y_global,ref_z_global,'r.',markersize=5,label='reference(end of bucket)')
        
    ## plot real path
    # file_suffix = '*path'+str(path_id)+'_rela_xz.csv'
    file_suffix = '0117161813_path0_rela_xz'+'.csv'
    for file in glob.glob(file_suffix):
        ...
        ## plot real path
        ax.plot3D(real_x_global,real_y_global,real_z_global,'b-',label='real(end of EE)')
